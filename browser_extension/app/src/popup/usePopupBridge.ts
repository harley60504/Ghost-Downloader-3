...
export function usePopupBridge(activeView: PopupView) {
  ...
  const saveDomainBlacklist = useCallback(async (value: string) => {
    try {
      await sendActionCommand({ type: "popup_set_domain_blacklist", value }, "儲存網域黑名單失敗");
      setPayload((current) => ({ ...current, domainBlacklist: value }));
      showToast("網域黑名單已儲存", "success");
      return true;
    } catch (error) {
      showToast(errorMessageOr(error, "儲存網域黑名單失敗"), "error");
      return false;
    }
  }, [showToast]);

  const saveTypeBlacklist = useCallback(async (value: string) => {
    try {
      await sendActionCommand({ type: "popup_set_type_blacklist", value }, "儲存類型黑名單失敗");
      setPayload((current) => ({ ...current, typeBlacklist: value }));
      showToast("類型黑名單已儲存", "success");
      return true;
    } catch (error) {
      showToast(errorMessageOr(error, "儲存類型黑名單失敗"), "error");
      return false;
    }
  }, [showToast]);

  const saveSizeBlacklist = useCallback(async (value: string) => {
    const normalized = value.trim();
    try {
      await sendActionCommand({ type: "popup_set_size_blacklist", value: normalized }, "儲存大小黑名單失敗");
      setPayload((current) => ({ ...current, sizeBlacklistMB: normalized }));
      showToast("大小黑名單已儲存", "success");
      return true;
    } catch (error) {
      showToast(errorMessageOr(error, "儲存大小黑名單失敗"), "error");
      return false;
    }
  }, [showToast]);

  const setNotifyOnTaskCreated = useCallback(async (enabled: boolean) => {
    ...
    try {
      ...
      showToast(errorMessageOr(error, "更新任務通知設定失敗"), "error");
    } finally {
      if (mountedRef.current) { setIsUpdatingNotifyOnTaskCreated(false); }
    }
  }, [applyPopupState, requestView, showToast]);
