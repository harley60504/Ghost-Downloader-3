...
async function readSettingsFromStorage(): Promise<PopupSettingsState> {
  ...
  return {
    ...
    mediaDownloadOverlayEnabled: Boolean(stored[MEDIA_DOWNLOAD_OVERLAY_KEY] ?? true),
...
export function usePopupBridge(activeView: PopupView) {
    ...
    const handleStorageChange = (
      ...
      setPayload((current) => {
        ...
        if (changes[MEDIA_DOWNLOAD_OVERLAY_KEY]) {
          next = { ...next, mediaDownloadOverlayEnabled: Boolean(changes[MEDIA_DOWNLOAD_OVERLAY_KEY].newValue ?? true) };
        }
  ...
  const requestPairing = useCallback(async () => {
    ...
    try {
      ...
      setFlash(result.message || "配对成功", "success");
      return true;
    } catch (error) {
  ...
  }, [refreshState, requestView, setFlash]);
  ...
  const setMediaDownloadOverlay = useCallback(
    async (enabled: boolean) => {
      ...
      try {
        ...
        setPayload((current) => ({ ...current, mediaDownloadOverlayEnabled: enabled }));
        try {
          const next = await sendRuntimeMessage<PopupStatePayload>({
