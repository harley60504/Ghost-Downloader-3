...
type TaskCreatedCallback = (message?: string) => void | Promise<void>;
type DownloadBlockPredicate = (input: { url: string; filename?: string; mime?: string; size?: number }) => boolean;
...
export function createResourceBridge(options: {
  ...
  function isLikelyFirefoxDownloadResponse(
    ...
    if (!isCapturableUrl(details.url)) { return false; }
    const statusCode = Number(details.statusCode ?? 0);
    if (statusCode < 200 || statusCode >= 400) { return false; }

    const disposition = responseHeaderValue(details.responseHeaders, "content-disposition").toLowerCase();
    if (disposition.includes("attachment") || meta.filename) { return true; }
    ...
    return Boolean(extension || mime) && !mime.startsWith("text/html");
  }
  ...
  function prepareFirefoxWebRequestHandoff(
    ...
    const meta = toResponseMeta(details.responseHeaders);
    meta.mime = mimeFromUrl(details.url) || meta.mime;
    if (!isLikelyFirefoxDownloadResponse(details, meta)) { return null; }

    const finalUrl = details.url;
    const headerSnapshot = cache.headerSnapshotByUrl(finalUrl);
    const matchedResource = cache.resourceByUrl(finalUrl) ?? undefined;
    const headers = { ...(headerSnapshot?.headers ?? {}) };
    const firefoxDetails = details as chrome.webRequest.OnHeadersReceivedDetails & { originUrl?: string };
    const referer = headers.referer
      || (firefoxDetails.originUrl && firefoxDetails.originUrl !== "null" ? firefoxDetails.originUrl : "")
    ...
    return {
      finalUrl,
      filename: meta.filename || basenameOf(matchedResource?.filename ?? "") || basenameOf(filenameFromUrl(finalUrl)) || "resource",
      mime: meta.mime || matchedResource?.mime || "",
      headers,
      size: meta.size > 0 ? meta.size : matchedResource?.size ?? 0,
      supportsRange: Boolean(meta.supportsRange || headerSnapshot?.supportsRange || matchedResource?.supportsRange),
      matchedResource,
    };
  }

  async function handoffPreparedDownload(prepared: PreparedDownloadHandoff): Promise<CommandResult> {
    if (shouldBlockPreparedDownload(prepared)) {
      return { ok: false, message: "下載已被黑名單規則忽略" };
    }
    try {
      ...
      if (result.ok) {
        if (prepared.matchedResource) { cache.setSent(prepared.matchedResource.id); }
        const message = result.message || `已攔截下載並加入任務：${prepared.filename}`;
      ...
      return { ok: false, message: error instanceof Error ? error.message : "下載接管失敗" };
    }
  }
  ...
  function tryInterceptFirefoxDownload(
    ...
    if (!prepared || shouldBlockPreparedDownload(prepared)) { return undefined; }
  ...
  async function routeBrowserDownload(downloadItem: chrome.downloads.DownloadItem) {
    ...
    const matchedResource = cache.resourceByUrl(finalUrl) ?? cache.resourceByUrl(downloadItem.url) ?? undefined;
    const filename =
    ...
    await handoffPreparedDownload({
      ...
      headers,
      size: typeof downloadItem.totalBytes === "number" && downloadItem.totalBytes > 0
