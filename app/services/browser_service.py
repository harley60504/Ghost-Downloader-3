...
class BrowserService(QObject):
    ...
    def __init__(self, parent=None):
        ...
        cfg.browserExtensionPort.valueChanged.connect(lambda _: self.restart())
        cfg.browserExtensionLanMode.valueChanged.connect(lambda _: self.restart())
    ...
    def start(self) -> None:
        ...
        address = QHostAddress.SpecialAddress.AnyIPv4 if cfg.browserExtensionLanMode.value else QHostAddress.SpecialAddress.LocalHost
        if self._server.listen(address, port):
            logger.info("Browser extension server started on {}:{}", self._server.serverAddress().toString(), port)
